...
A recomendação que vou defender é manter a skill como unidade de workflow e extrair somente os componentes que realmente merecem ser compartilhados.

**Transição:** primeiro precisamos entender os dois extremos que queremos evitar.

## Slide 2 — Duplicação versus shared caótico
Se cada skill for completamente isolada, rapidamente vamos repetir scripts, guidelines e templates. Isso aumenta manutenção e faz pequenas diferenças aparecerem entre skills que deveriam seguir o mesmo padrão.
...
O reuso não precisa acontecer reaproveitando uma skill inteira. Ele pode acontecer em componentes menores: scripts compartilhados, referências compartilhadas e assets compartilhados. Isso preserva contexto local e evita criar mega skills genéricas.

**Transição:** essa ideia aparece diretamente no formato do monorepo.

## Slide 4 — Layout recomendado do monorepo
Cada skill mantém seu package local: SKILL.md, scripts, references e assets. Isso é importante porque mantém a arquitetura que já discutimos e deixa claro o que pertence àquele workflow.

Fora das skills existe um shared controlado. shared/scripts recebe execução realmente reutilizável. shared/references recebe padrões transversais. shared/assets recebe templates e artefatos reutilizáveis. E tooling cuida da qualidade do ecossistema.
...
Uma skill pode ter arquivos locais quando eles são específicos daquele workflow. Se existe uma regra exclusiva de cartões para PR review, não há motivo para colocar isso no shared só porque temos um monorepo.

Autonomia conceitual não significa isolamento absoluto. Significa que eu consigo abrir a skill e entender o que ela faz.
...
Mas existe uma regra: só subir para shared quando realmente houver uso transversal. Um script exclusivo de uma skill continua no package daquela skill.

Isso evita duplicação sem criar uma camada de infraestrutura genérica antes da hora.
...
shared/assets guarda templates, exemplos e outros artefatos reutilizáveis. Por exemplo, um template padrão de PR review ou ADR.
...
O anti-pattern é transformar a execução numa caça ao tesouro: script chama outro script, que depende de uma referência escondida, que por sua vez muda comportamento de outra parte.
...
Para decisão, ela pode consultar references locais e referências compartilhadas. Para saída, usa um template em shared/assets.
...
Também precisamos saber impacto: se um script compartilhado muda, quais skills dependem dele? Esse tipo de rastreabilidade precisa aparecer cedo, antes do monorepo crescer demais.
...
**Transição:** agora conseguimos resumir a recomendação em poucas linhas.
...
A recomendação é manter as dependências explícitas e extrair shared somente depois de existir evidência de repetição.

**Transição:** por fim, as referências usadas para chegar nessa proposta.

## Slide 13 — Referências e agradecimentos
A estrutura proposta combina o modelo de package com SKILL.md e recursos adicionais, o princípio de progressive disclosure e os conceitos de decomposição e reconstrução discutidos em Workflow-to-Skill.
