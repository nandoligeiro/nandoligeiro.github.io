...
export default function BlogPost() {
  ...
  if (!post) {
    return (
      <div className="page" style={{ textAlign: 'center', paddingTop: '5rem' }}>
        <h1 style={{ color: 'var(--primary)', fontFamily: 'Space Grotesk' }}>Post não encontrado</h1>
        <Link to="/blog" className="btn btn-outline" style={{ marginTop: '1.5rem' }}>← Voltar ao Blog</Link>
      </div>
    )
  }
  ...
  return (
    <>
      <Helmet>
        <title>{post.title} | Fernando Costa</title>
      ...
      <div className="page" style={{ maxWidth: '760px' }}>
        {/* Back */}
        <Link to="/blog" style={{ color: 'var(--text-muted)', fontSize: '0.875rem', display: 'inline-flex', alignItems: 'center', gap: '0.35rem', marginBottom: '2rem' }}>
          ← Blog
        </Link>
        ...
        {/* Content — HTML pre-processed at build time */}
